...
void LCD_PrintFloat(float num){
  // prints until 2 decimal places
  // assumed passed value is always less than 1000.
  unsigned char sign = ' ';
  unsigned char tempString[16];
  unsigned char tempString_Decimal[2];
  unsigned short int tempNum;
  unsigned char decimalIndex = 0;
  unsigned char integerIndex = 0;
  
  if (num < 1000) {
    tempNum = (int)(num * 100);
    
    // If our number is 0, print 0.00
    if (num == 0) {
      LCD_PrintChar('0');
      LCD_PrintChar('.');
      LCD_PrintChar('0');
    ...
    // Build our number string via repeated division
     // Save fraction part first
    while (decimalIndex < 2) {
        tempString_Decimal[decimalIndex] = (tempNum % 10) + 48;
        tempNum = tempNum / 10;
        decimalIndex++;
      }
    // Save integer part next
    while (tempNum > 0) {
      tempString[integerIndex] = (tempNum % 10) + 48;
      tempNum = tempNum / 10;
      integerIndex++;
    }
    ...
    // Print out our string in reverse order
    while (integerIndex) {
      LCD_PrintChar(tempString[integerIndex-1]);
      integerIndex--;
    }
    ...
    while (decimalIndex){
      LCD_PrintChar(tempString_Decimal[decimalIndex-1]);
      decimalIndex--;
    }
  }
}
